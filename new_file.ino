...
//      moveStop();
      ...
      {
        Serial.println("going LEFT BCCCCCCCCC");
        turnLeft();
        moveStop();
      }
    }
...
void fire_ex()
{
  ...
  Serial.println("nhi h aag");
  digitalWrite(pump, LOW);
  motor1.run(BACKWARD);
  motor2.run(BACKWARD);
  motor3.run(BACKWARD);
  motor4.run(BACKWARD);
  Serial.println("going peeche");
  delay(1000);
  motor1.run(FORWARD);
  motor2.run(FORWARD);
  motor3.run(FORWARD);
  motor4.run(FORWARD);
  Serial.println("going aage");
  myservo.write(90);
  }
}
